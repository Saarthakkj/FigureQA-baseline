...
def conv2d(inputs, num_channels, is_training, activation_fn=tf.nn.relu,
    ...
    with tf.variable_scope(name):
        ...
        if use_dropout:
            conv = tf.keras.layers.Dropout(keep_prob)(conv, training=is_training)
        if activation_fn is None:
            activation_fn = lambda x: x
        activation = activation_fn(conv + biases)
        if use_batch_norm:
            return tf.keras.layers.BatchNormalization(
                center=True, scale=True, training=is_training)(activation)
        else:
            return activation
...
def fc(inputs, num_outputs, is_training, activation_fn=tf.nn.relu,
    ...
    with tf.variable_scope(name):
        h = tf.keras.layers.Dense(
            num_outputs, activation=None,
            **kwargs)(inputs)
        if use_dropout:
            h = tf.keras.layers.Dropout(keep_prob)(h, training=is_training)
        if activation_fn is None:
            activation_fn = lambda x: x
        h = activation_fn(h)
        if use_batch_norm:
            return tf.keras.layers.BatchNormalization(
                center=True, scale=True, training=is_training)(h)
        else:
            return h
